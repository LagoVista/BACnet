    ...
    public class BacnetPtpProtocolTransport : BacnetTransportBase
    {
        ...
        private void RemoveGarbage(byte[] buffer, ref int length)
        {
            ...
            for (var i = 0; i < length - 1; i++)
            {
                ...
                length -= i;
                Log.AddCustomEvent(LagoVista.Core.PlatformSupport.LogLevel.Message, "BacnetPtpProtocol", "Garbage");
                return;
            }
        ...
        private void PTPThread()
        {
            ...
            try
            {
                while (_port != null)
                {
                    ...
                    switch (status)
                    {
                        ...
                        case BacnetMstpProtocolTransport.GetMessageStatus.ConnectionError:
                            Log.AddCustomEvent(LagoVista.Core.PlatformSupport.LogLevel.Warning, "BacnetPtpProtocol", "Connection disturbance");
                            Reconnect();
                            break;

                        case BacnetMstpProtocolTransport.GetMessageStatus.DecodeError:
                            Log.AddCustomEvent(LagoVista.Core.PlatformSupport.LogLevel.Warning, "BacnetPtpProtocol", "PTP decode error");
                            break;

                        case BacnetMstpProtocolTransport.GetMessageStatus.SubTimeout:
                            Log.AddCustomEvent(LagoVista.Core.PlatformSupport.LogLevel.Warning, "BacnetPtpProtocol", "PTP frame abort");
                            break;
                        ...
                        case BacnetMstpProtocolTransport.GetMessageStatus.Good:
                            ...
                            switch (frameType)
                            {
                                ...
                                case BacnetPtpFrameTypes.FRAME_TYPE_DISCONNECT_REQUEST:
                                    ...
                                    Log.AddCustomEvent(LagoVista.Core.PlatformSupport.LogLevel.Message, "BacnetPtpProtocol", $"Disconnect requested: {reason}");
                                    Reconnect();
                                    break;
                ...
                Log.AddCustomEvent(LagoVista.Core.PlatformSupport.LogLevel.Message, "BacnetPtpProtocol", "PTP thread is closing down");
            }
            catch (Exception ex)
            {
                Log.AddException($"Exception in PTP thread", ex);
            }
        }
    }
}