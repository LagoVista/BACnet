    ...
    public class BacnetMstpProtocolTransport : BacnetTransportBase
    {
        ...
        private void MstpThread()
        {
            try
            {
                ...
                while (_port != null)
                {
                    Log.AddCustomEvent(LagoVista.Core.PlatformSupport.LogLevel.Message, "BacnetMstpProtocol_StateChanged", stateChange.ToString());
                ...
                Log.AddCustomEvent(LagoVista.Core.PlatformSupport.LogLevel.Message, "BacnetMstpProtocol", "MSTP thread is closing down");
            }
            catch (Exception ex)
            {
                Log.AddException("Exception in MSTP thread", ex);
            }
            ...
            out byte destinationAddress, out byte sourceAddress, out int msgLength)
        {
            ...
            Log.AddCustomEvent(LagoVista.Core.PlatformSupport.LogLevel.Message, "BacnetMstpProtocol", $"{frameType} {destinationAddress:X2}");
